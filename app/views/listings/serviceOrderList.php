				...
				<?php if (!empty($serviceOrders)): ?>
					<?php foreach ($serviceOrders as $so): ?>
						<tr id="row-<?= $so['id'] ?>">
							...
							<td id="nameTd"><?= htmlspecialchars($so['name'] ?? 'N/A') ?></td>
							<td id="cpfTd"><?= htmlspecialchars($so['cpf']  ?? '') ?></td>
							<td id="cnpjTd"><?= htmlspecialchars($so['cnpj'] ?? '') ?></td>
							<td><?= htmlspecialchars($so['age'] ?? '') ?></td>
							<td><?= date('d/m/Y H:i:s', strtotime($so['inserted_at'])) ?></td>
							<td id="actionTd">
								<a id="btnEdit" href="<?= BASE_URL ?>/client/edit/<?= htmlspecialchars($so['id'], ENT_QUOTES, 'UTF-8') ?>" data-direction="right">
...
<script>
	...
	async function confirmRemoval(id) {
		try {
			const resp = await fetch(`<?= BASE_URL ?>/client/remove/${id}`, {
				method: 'POST',
				headers: {
