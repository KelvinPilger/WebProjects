<div class="tableContainer">
	<div id="headerButtons">
		<strong>
			<a href="<?= BASE_URL ?>/serviceorder/create" class="btnIncluir" direction="right" id="btnIncluir">
				Nova OS
			</a>
		</strong>
	</div>
	<table class="myTable">
		<form id="filterForm" method="get" action="<?= BASE_URL ?>/serviceorder/index">
			<input
				type="hidden"
				name="page"
				id="pageInput"
						...
						<a class="paginationBtn" href="?page=<?= 1 ?>&rowLimit=<?= $rowLimit ?>">
							<<</a>
								...
								<a class="paginationBtn" href="?page=<?= $currentPage - 1 ?>&rowLimit=<?= $rowLimit ?>">
									...
									<?php if ($rowLimit === 'all'): ?>
										1 - <?= htmlspecialchars($total) ?> de <?= htmlspecialchars($total) ?> Ordens
									<?php elseif ($total > 0): ?>
										<?= htmlspecialchars($offset + 1) ?>
										-
										<?= htmlspecialchars(min($offset + $limit, $total)) ?>
										de <?= htmlspecialchars($total) ?> Ordens
									<?php else: ?>
			...
			<tbody>
				<?php if (!empty($serviceOrders)): ?>
					<?php foreach ($serviceOrders as $so): ?>
						<tr id="row-<?= $so['id'] ?>">
							<td id="idTd"><?= htmlspecialchars($so['id']) ?></td>
							<td id="nameTd"><?= htmlspecialchars($so['client_name'] ?? 'N/A') ?></td>
							<td id="cpfTd"><?= htmlspecialchars($so['total_value']  ?? '') ?></td>
							<td id="cnpjTd"><?= date('d/m/Y H:i:s', strtotime($so['forecast_date'])) ?></td>
							<td><?= htmlspecialchars($so['age'] ?? '') ?></td>
							<td><?= htmlspecialchars($so['situation']) ?></td>
							<td id="actionTd">
								<a id="btnEdit" href="<?= BASE_URL ?>/serviceorder/edit/<?= htmlspecialchars($so['id'], ENT_QUOTES, 'UTF-8') ?>" data-direction="right">
								...
								<button id="btnExcluir" type="button" onclick="javascript:openRemoveModal(<?= htmlspecialchars($so['id'] ?? null) ?>)">
...
<script>
	...
	async function confirmRemoval(id) {
		try {
			const resp = await fetch(`<?= BASE_URL ?>/serviceorder/remove/${id}`, {
				method: 'POST',
				headers: {
